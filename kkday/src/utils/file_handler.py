...
def ensure_directory_structure(city_name):
    ...
    try:
        ...
        if city_name == country:
            ...
            # 일반 도시: 대륙/국가/도시 구조
            img_dir = os.path.join("kkday_img", continent, country, city_name)
...
def get_dual_image_urls_kkday(driver, url_type="Product"):
    """KKday에서 메인 이미지와 썸네일 이미지 URL 추출 (원본 정교한 셀렉터 사용)"""
...
def download_single_image_kkday(img_src, product_number, city_name, image_type="main", max_size_kb=300):
...
def download_dual_images_kkday(image_urls, product_number, city_name, max_size_kb=300):
    ...
    if image_urls.get("main"):
        ...
        main_filename = download_single_image_kkday(
            image_urls["main"], 
            product_number, 
            city_name, 
            image_type="main",
    ...
    if image_urls.get("thumb"):
        ...
        thumb_filename = download_single_image_kkday(
            image_urls["thumb"], 
            product_number, 
            city_name, 
            image_type="thumb",
...
def find_project_root():
    """kkday 프로젝트 루트 디렉토리를 자동으로 찾기 (어디서 실행되든 작동)"""
    current_path = os.path.abspath(os.getcwd())
    
    # 현재 위치부터 시작해서 상위로 올라가며 kkday 프로젝트 찾기
    search_path = current_path
    max_depth = 10  # 무한루프 방지
    
    for i in range(max_depth):
        # kkday 프로젝트 특징적 파일/폴더들 확인
        indicators = [
            'kkday_img',           # 이미지 폴더
            'src',                 # 소스 코드 폴더
            'data',                # 데이터 폴더
            'KKday_Crawler_v1.ipynb'  # 노트북 파일
        ]
    ...
    print(f"kkday 프로젝트 루트를 찾지 못했습니다. 현재 위치 사용: {current_path}")
    return current_path
...
def get_smart_image_path(city_name, product_number, image_type="main"):
    ...
    try:
        ...
        if city_name == country:  # 도시국가
            relative_path = os.path.join("kkday_img", continent, country, filename)
        else:  # 일반 도시
            relative_path = os.path.join("kkday_img", continent, country, city_name, filename)
...
def download_and_save_image_kkday(image_url, product_number, city_name, image_type="main", max_size_kb=300):
    """하위 호환성을 위한 래퍼 함수"""
    return download_single_image_kkday(image_url, product_number, city_name, image_type, max_size_kb)

def get_image_stats(city_name):
    """이미지 저장 통계"""
    try:
        img_dir = os.path.join("kkday_img", city_name)
...
def create_country_consolidated_csv(country_name, force_recreate=False):
    ...
    try:
            ...
            if os.path.exists(continent_country_path):
                ...
                for city in os.listdir(continent_country_path):
                    ...
                    if os.path.isdir(city_path):
                        # 도시별 CSV 찾기
                        csv_file = os.path.join(city_path, f"kkday_{city}_products.csv")
        ...
        if country_name in city_countries:
            for continent in search_continents:
                city_csv = os.path.join(data_base, continent, f"kkday_{country_name}_products.csv")
        ...
        if country_continent:
            ...
            if country_name in city_countries:
                # 도시국가: 대륙 직하에 저장
                consolidated_path = os.path.join(data_base, country_continent, f"{country_name}_통합_kkday_products.csv")
            else:
                # 일반 국가: 대륙/국가/ 경로에 저장
                country_dir = os.path.join(data_base, country_continent, country_name)
                os.makedirs(country_dir, exist_ok=True)
                consolidated_path = os.path.join(country_dir, f"{country_name}_통합_kkday_products.csv")
        else:
            # 대륙 정보가 없는 경우: 기타 폴더에 저장
            other_dir = os.path.join(data_base, "기타", country_name)
            os.makedirs(other_dir, exist_ok=True)
            consolidated_path = os.path.join(other_dir, f"{country_name}_통합_kkday_products.csv")
